...
public class GameManager {
		...
		while (i.hasNext()) {
			...
			for (final Player player : Bukkit.getServer().getOnlinePlayers()) {
				if (player.getUniqueId() == uuid) {
					...
					api.getMessageUtil().sendMessage(player, api.getMessageUtil().getMessage(plantsWon ? "plants won" : "zombies won"));
				}
			}
		}
		Iterator<UUID> i2 = game.getZombies().getMembers().keySet().iterator();
	...
	public void updateRows(Game game) {
		...
		for (int i = 0; i < game.getFarm().getRows().size(); i++) {
			...
			TempRow tempRow = new TempRow(row.getZombieSpawn(), row.getPlantSpawn(), row.getEndpoint());
			temps.add(tempRow);
		}
		game.setRows(temps);
	}
