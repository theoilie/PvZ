...
public class Validate {
	...
	public boolean hasPerm(Player player, String perm) {
		if (player.hasPermission(perm))
			return true;
		Main.getAPI().getMessageUtil().sendMessage(player, Main.getAPI().getMessageUtil().getMessage("no permission"));
		return false;
	}
	...
	public boolean hasSelection(Player player) {
		if (Selection.getPlayerSelection(player).areBothPointsSet() && Selection.getPlayerSelection(player).areBlocksInSameWorld())
			...
			Main.getAPI().getMessageUtil().sendMessage(player, Main.getAPI().getMessageUtil().getMessage("selection not set properly"));
			return false;
		}
	}
