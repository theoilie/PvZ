...
public class InventoryManager {
	 ...
	 * @param plantType the plant type the player is, or null if he/she is a zombie
	 * @param zombieType the zombie type the player is, or null if he/she is a plant
	...
	public void giveSpectatingInventory(final Player player, final PlantType plantType, final ZombieType zombieType, final Game game) {
		...
		api.getMessageUtil().sendMessage(player, api.getMessageUtil().getMessage("respawn"));
	...
	private ItemStack createItem(ConfigurationSection itemSection) {
		@SuppressWarnings("deprecation")
		ItemStack item = new ItemStack(itemSection.getInt("item id"), itemSection.getInt("item amount"));
		return item;
	}
	...
	private void findRow(Player player, Game game, boolean plant) {
		...
		if (row == null) {
			api.getMessageUtil().sendMessage(player,api.getMessageUtil().getMessage("no rows created"));
			return;
		}
		try {
			if (plant) {
			...
			api.getMessageUtil().sendMessage(player, api.getMessageUtil().getMessage("no spawn set"));
		}
